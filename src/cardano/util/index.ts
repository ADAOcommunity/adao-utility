...
import { Lovelace, Utxo } from '@cardano-ogmios/schema';
import {
  Address,
  PlutusData,
...
// -- Basic Utilities
async function getUtxos(address : A.Address) : Promise<Utxo> {
  return await fetch("https://api.koios.rest/api/v0/address_info?_address=" + address, {
    method: "GET",
  }).then((res) => res.json());
}

async function blockfrostRequest(base, endpoint, headers, body) {
...
async function getBalance(address : A.Address) : Promise<Lovelace> {
  return await fetch("https://api.koios.rest/api/v0/address_assets?_address=" + address, {
    method: "GET",
  }).then((res) => res.json());
}
...
async function depositToTreasury(treasuryAddress, requestedAmount) {
  ...
  const treasuryAddr = Loader.Cardano.BaseAddress.from_address(
    Loader.Cardano.Address.from_bech32(
      treasuryAddress
    )
  );
...
class Escrow {
  ...
  utxoMatches(utxoAssets, offerAssets) {
    // For each unit -> quantity in the offerAssets there must be the same thing in utxo.
    // If the offer contains lovelace we consider this as well.
    let falsified = true;
    console.log(JSON.stringify(utxoAssets))
    console.log(JSON.stringify(offerAssets))
    offerAssets.forEach((asset) => {
      console.log(JSON.stringify(asset));
      let utxoAsset = utxoAssets.find(iasset => iasset.unit == asset.unit)
      if (utxoAsset === undefined) {
        console.log("This should be reached as well.. and first.")
        falsified = false;
        return false;
      }
      console.log("Utxo Asset: " + utxoAsset.unit + "  Offer Asset: " + asset.unit);
      if (parseInt(utxoAsset.quantity) < parseInt(asset.quantity)) {
        console.log("This should be reached right?!")
        falsified = false;
        return false;
      }
      console.log(JSON.stringify(asset))
      console.log(JSON.stringify(utxoAsset))
      console.log(parseInt(utxoAsset.quantity))
      console.log(parseInt(asset.quantity))
    });
    return falsified;
  }
 // curl -X GET "https://api.koios.rest/api/v0/address_info?_address=addr1wxaptpmxcxawvr3pzlhgnpmzz3ql43n2tc8mn3av5kx0yzs09tqh8" \
  /**
  async function getBalance(addrs : A.Address[]) : Promise<Lovelace> {
    const utxo = await client.utxo(addrs, {port: 1337})
    return utxo.reduce((total, [_, out]) => total + out.value.coins, 0)
  } */
   ...
   */
  async getUtxo(offer) {
    const utxos = await getUtxos(CONTRACT_ADDRESS().to_bech32());
